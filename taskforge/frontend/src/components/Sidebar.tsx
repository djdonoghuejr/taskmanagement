  ...
  return (
    <aside className="w-72 shrink-0 border-r border-slate-200 bg-white/80 p-4 backdrop-blur">
      <div className="rounded-2xl bg-gradient-to-br from-slate-900 to-slate-700 p-4 text-white">
        ...
        {user ? <p className="mt-3 truncate text-xs text-slate-200">{user.email}</p> : null}
      </div>
      ...
      <div className="mt-6 border-t border-slate-200 pt-4">
        <button
          className="w-full rounded-lg px-3 py-2 text-left text-sm font-medium text-slate-700 hover:bg-slate-200"
          onClick={() => void logout()}
        >
          Log out
        </button>
