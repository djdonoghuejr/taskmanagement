...
def list_tags(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return (
        db.query(Tag)
        .filter(Tag.user_id == user.id, Tag.is_archived == False)
...
def update_tag(tag_id: UUID, payload: TagUpdate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def delete_tag(tag_id: UUID, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    tag = db.query(Tag).filter(Tag.id == tag_id, Tag.user_id == user.id).first()
    if not tag:
        raise HTTPException(status_code=404, detail="Tag not found")
